...
/**
 * @file
 * Contains \Drupal\evanced_aggregator\Controller\EvancedAggregatorController.
 */

namespace Drupal\evanced_aggregator\Controller;
 ...
 */
class EvancedAggregatorController extends ControllerBase {
  ...
  protected function buildPageList(array $items, $feed_source = '') {
    ...
    if ($items) {
      $build['items'] = $this->entityManager()->getViewBuilder('evanced_item')
  ...
  public function adminOverview() {
    ...
    foreach ($feeds as $feed) {
      $row = array();
      $row[] = $feed->link();
      $row[] = $this->formatPlural($entity_manager->getStorage('evanced_item')->getItemCount($feed), '1 item', '@count items');
  ...
  public function pageLast() {
    $items = $this->entityManager()->getStorage('evanced_item')->loadAll(20);
